...
const getUsersWithAge = (arr, min, max) => {
  return arr.filter(arr => min < arr.age && arr.age < max);
};
...
const getTotalBalance = arr => {
  return arr.reduce((acc, arr) => acc + arr.balance, 0);
};
...
const getUsersByFriend = (arr, name) => {
  return arr
    .filter(arr => (arr.friends.find(nam => nam === name) ? arr.name : ''))
    .map(usersByFriend => usersByFriend.name);
};
...
const getAllSkills = arr => {
  const userSkills = arr.reduce(
    (acc, x) => (x.skills ? acc.concat(x.skills) : acc),
    [],
  );
  userSkills.sort();
  return uniqueArray(userSkills);
};

//функция исключает повторяющиеся элементы массива
function uniqueArray(arrUnigue) {
  return arrUnigue.reduce((acc, value) => {
...
const getUserNamesSortedByFriendsCount = arr => {
  const sortUserFriends = (a, b) => a.friends.length - b.friends.length;
  return arr.sort(sortUserFriends).map(arr => arr.name);
};
