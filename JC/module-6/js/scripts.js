...
class Hamburger {
  ...
  removeTopping(topping) {
    this._topping = this._topping.filter(item => item !== topping);
  }
  /**
   ...
   */
  get calculatePrice() {
    const priceSize = Hamburger.SIZES[this._size].price;
    const priceStuffing = Hamburger.STUFFINGS[this._stuffing].price;

    const priceTopping = this._topping.map(
      value => (value = Hamburger.TOPPINGS[value].price),
    );
    const pricesTopping = priceTopping.reduce((acc, value) => acc + value, 0);

    return priceSize + priceStuffing + pricesTopping;
  }
  ...
  get calculateCalories() {
    const caloriesSize = Hamburger.SIZES[this._size].calories;
    const caloriesStuffing = Hamburger.STUFFINGS[this._stuffing].calories;

    const calorieTopping = this._topping.map(
      value => (value = Hamburger.TOPPINGS[value].calories),
    );
    const caloriesTopping = calorieTopping.reduce((acc, value) => acc + value, 0);
